...
bool searchTitle(RecordingsItemRec *&RecItem, const std::vector<RecordingsItemRec*> *RecItems, const cEvent *event, cScraperVideo *scraperVideo) {
  if (RecItems->empty() ) return false;                         // there are no recordings
  if (isEmptyOrWhitespace(cSv(event->Title()))) return false;   // event has no title
...
// find all recordings with equal title
  ...
  if (!isEmptyOrWhitespace((*equalTitle.first)->Folder())) {
    cSplit folders((*equalTitle.first)->Folder(), '~', eSplitDelimBeginEnd::optional);
    cSplit<cSv>::const_iterator item = --folders.end();
    int result = compare_utf8_lower_case_ignore_punct(*item, dummy->Title());
    if (!result) return false;                                  // matches folder of a series
  }
...
template<typename C>
  const_rec_iterator<C> &const_rec_iterator<C>::set_backwards(bool backwards) {
    m_backwards = backwards;
    return set_begin();
  }
template<typename C>
  const_rec_iterator<C> &const_rec_iterator<C>::set_container(const std::vector<C> &items) {
    m_begin = items.begin();
    m_end   = items.end();
    return set_begin();
  }
...
template class const_rec_iterator<RecordingsItemDirPtr>;
...
// check availability of scraper data
    ...
    scraperDataAvailable |= LiveSetup().GetShowSpecialFolders();
...
// create "base" folders
    if (scraperDataAvailable) {
      ...
      m_root->m_cmp_rec = RecordingsItemPtrCompare::ByAscendingNameDescSort;  // there are no recordings -> dummy
// TV shows
...
// add file system directories
    ...
    if (scraperDataAvailable) {
// delete folders with less than 2 recordings
